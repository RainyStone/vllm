"""Length-bucketed DP load balancer (ported from PR #44967).

Pure-Python (numpy only): buckets requests by sequence length and assigns them
to engines with load- + length-affinity-aware redirection. In the flat
``dycp_size`` scheme the integration point is
``DPLBAsyncMPClient.get_core_engine_for_request`` (the least-loaded engine
selection, currently ``score = waiting*4 + running``): consult the balancer for
the engine index when an opt-in flag is set. Wiring is deferred pending runtime
validation on a multi-engine deployment (no HW available here).
"""
import random
