"""Cross-DP KV cache manager (ported from PR #44967).

Comm-scheme-agnostic: takes ``cp_world_size`` and manages one block pool per
CP/DYCP rank for cross-DP prefix caching. In the flat ``dycp_size`` scheme the
integration point is the scheduler's KV-cache-manager construction
(``CPAwareScheduler`` extends ``vllm.v1.core.sched.scheduler.Scheduler``): wire
``CrossDPKVCacheManager(..., cp_world_size=parallel_config.dycp_size)`` there
behind an opt-in config flag. Full wiring is deferred pending runtime
validation on a multi-DP deployment (no HW available here).
"""
